...
class EmojidecoratorValidationException implements Exception {
  final List<String> errors;

  EmojidecoratorValidationException(this.errors);

  @override
  String toString() => 'EmojidecoratorValidationException: ${errors.join("; ")}';
}
...
/// Emoji Decorator API client.
///
/// For more information, visit: https://apiverve.com/marketplace/emojidecorator?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [text] (required) - The text you want decorated with emojis
class EmojidecoratorClient {
  final String apiKey;
  ...
  EmojidecoratorClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/emojidecorator',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [EmojidecoratorValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw EmojidecoratorValidationException(errors);
    }
  }

  /// Execute a request to the Emoji Decorator API.
  ///
  /// Parameters are validated before sending the request.
  Future<EmojidecoratorResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw EmojidecoratorException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return EmojidecoratorResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw EmojidecoratorException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw EmojidecoratorException('Resource not found');
      } else {
        throw EmojidecoratorException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is EmojidecoratorException) rethrow;
      throw EmojidecoratorException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Emoji Decorator API client.
class EmojidecoratorException implements Exception {
  final String message;

  EmojidecoratorException(this.message);

  @override
  String toString() => 'EmojidecoratorException: $message';
}
